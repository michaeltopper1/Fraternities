...
alc_main <- list(alc_1, alc_2, alc_3, alc_4_weekend, alc_5_weekday)
...
sex_boot <- map(sex_main, ~boottest(., param = "treatment", clustid = "university", B = 1000))

sex_boot_pvalues <- map(sex_boot, ~.x$p_val) %>% 
  unlist()
...
main_table <- ifc::main_table(alc_main, last_panel = sex_main) %>%
  ...
  add_row(term = "Wild Bootstrap P-Value",
          ...
          `Model 2` = sprintf("%.3f",sex_boot_pvalues[[1]]),
          `Model 3` = sprintf("%.3f",sex_boot_pvalues[[1]]),
          `Model 4` = sprintf("%.3f",sex_boot_pvalues[[1]]),
          `Model 5` = sprintf("%.3f",sex_boot_pvalues[[1]]),
          .before = 10) %>%
  kbl(booktabs = T,
