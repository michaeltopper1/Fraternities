...
passing_1 <- ifc::reghdfe(daily_crime_passing, "alcohol_offense_per25", explanatory_vars_1,
             fixed_effects_preferred, "university")
...
passing_6 <- ifc::reghdfe(daily_crime_passing %>% 
               filter(university %in% sixth_passing), "alcohol_offense_per25", explanatory_vars_6,
             fixed_effects_preferred, "university")
...
sex_passing <- map_df(list(passingsex_1, passingsex_2, passingsex_3,
                           passingsex_4, passingsex_5, passingsex_6), ~ .x %>% broom::tidy(conf.int = T),
                      .id = "var") %>% 
  mutate(passing = case_when(
    var == 1 ~ "37 Schools",
    var == 2 ~ "33 Schools",
...
alc_passing_g <- alc_passing %>% 
  ...
  scale_y_continuous(limits = c(-.5,.5), breaks = c(-.25, 0, .25)) +
  ...
  facet_wrap(~fct_rev(passing), ncol = 1, strip.position = "left") +
  theme_minimal()

sex_passing_g <- sex_passing %>% 
  mutate(term = factor(term, levels = 
                         c("weeks12_final",
                           "weeks34_final",
                           "weeks56_final",
  ...
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.1) +
  geom_hline(yintercept = 0, color = "dark red") +
  scale_x_discrete(labels = c("Weeks\n1 & 2",
                              "Weeks\n3 & 4",
                              "Weeks\n5 & 6",
                              "Weeks\n7 & 8",
                              "Weeks\n9 & 10",
