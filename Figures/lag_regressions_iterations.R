...
collapsed_data_daily_1 <- appended_crime_logs %>% 
  mutate(report_lag = date_reported - date_occurred) %>% relocate(report_lag) %>% 
  filter(report_lag >=0 | is.na(report_lag)) %>% 
  filter(!is.na(date_occurred)) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 1 & . == 1, 1,0), .names = "{.col}_lag_1")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 2 & . == 1, 1,0), .names = "{.col}_lag_2")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 3 & . == 1, 1,0), .names = "{.col}_lag_3")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 4 & . == 1, 1,0), .names = "{.col}_lag_4")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 5 & . == 1, 1,0), .names = "{.col}_lag_5")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 6 & . == 1, 1,0), .names = "{.col}_lag_6")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 7 & . == 1, 1,0), .names = "{.col}_lag_7")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 8 & . == 1, 1,0), .names = "{.col}_lag_8")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 9 & . == 1, 1,0), .names = "{.col}_lag_9")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 10 & . == 1, 1,0), .names = "{.col}_lag_10")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 11 & . == 1, 1,0), .names = "{.col}_lag_11")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 12 & . == 1, 1,0), .names = "{.col}_lag_12")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 13 & . == 1, 1,0), .names = "{.col}_lag_13")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 14 & . == 1, 1,0), .names = "{.col}_lag_14")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 15 & . == 1, 1,0), .names = "{.col}_lag_15")) %>% 
  mutate(across(c(sexual_assault, alcohol_offense), ~ifelse(report_lag > 16 & . == 1, 1,0), .names = "{.col}_lag_16")) %>% 
  mutate(report_lag_alc = ifelse(report_lag > 2 & alcohol_offense == 1, 1, 0)) %>% 
  mutate(report_lag_sex = ifelse(report_lag > 2 & sexual_assault == 1, 1, 0)) %>% 
  mutate(report_lag_drug = ifelse(report_lag > 2 & drug_offense == 1, 1, 0)) %>% 
  mutate(report_lag_rob = ifelse(report_lag > 2 & robbery_burglary == 1, 1, 0)) %>% 
  mutate(report_lag_theft = ifelse(report_lag > 2 & theft == 1, 1, 0)) %>% 
  ...
  select(date_reported, university, sexual_assault, alcohol_offense, drug_offense, theft, robbery_burglary, alcohol_offense_strict,
         noise_offense, rape, starts_with("report_lag_"), starts_with("sexual_assault_lag"), starts_with("alcohol_offense_lag")) %>% 
