...
theft_ols_1 <- daily_crime %>% 
  feols(theft_per25 ~week_before +  treatment + week_after | date + university,
        vcov = cluster ~ university, data = .)

theft_ols_2 <- daily_crime %>% 
  feols(theft_per25 ~ week_before + treatment+ week_after | day_of_week + university + semester_number,
        cluster = ~university, data = .)

theft_ols_3 <- daily_crime %>% 
  feols(theft_per25 ~ week_before + treatment + week_after| day_of_week + university_by_semester_number  ,
        cluster = ~university, data = .)
...
theft_weekend_1 <- daily_crime_weekends %>% 
  feols(theft_per25 ~ week_before + treatment + week_after | date + university,
        cluster = ~university, data = .) 
theft_weekend_2 <- daily_crime_weekends %>% 
  feols(theft_per25 ~ week_before + treatment + week_after | day_of_week + university + semester_number,
        cluster = ~university, data = .) 
theft_weekend_3 <- daily_crime_weekends %>% 
  feols(theft_per25 ~week_before +  treatment  + week_after| day_of_week + university_by_semester_number ,
        cluster = ~university, data = .)
...
theft_weekdays_1 <- daily_crime_weekdays %>% 
  feols(theft_per25 ~week_before +  treatment + week_after | date + university,
        cluster = ~university, data = .) 
theft_weekdays_2 <- daily_crime_weekdays %>% 
  feols(theft_per25 ~week_before + treatment + week_after| day_of_week + university + semester_number,
        cluster = ~university, data = .) 
theft_weekdays_3 <- daily_crime_weekdays %>% 
  feols(theft_per25 ~ week_before + treatment + week_after| day_of_week + university_by_semester_number ,
        cluster = ~university, data = .)


theft_ols <- list("(1)" = theft_ols_1, "(2)" = theft_ols_2, "(3)" = theft_ols_3,
                 "(1)" = theft_weekend_1, "(2)" = theft_weekend_2, "(3)" = theft_weekend_3, 
                 "(1)" = theft_weekdays_1, "(2)" = theft_weekdays_2, "(3)" = theft_weekdays_3)
...
theft_table <- modelsummary(theft_ols, stars = T,
             ...
             title = "\\label{theft_offense}Effect of Moratoriums on theft Offenses",
             notes = list("The sample includes 38 universities. Some universities go in and out of moratoriums multiple times.",
                          ...
                          "Outcome of interest is theft offenses per 25 thousand enrolled students.",
