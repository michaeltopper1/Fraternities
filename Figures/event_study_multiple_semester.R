## Purpose of script: creates the 'multiple event' event study using weekly data with semester number fixed effect and unviersity
##
...
es_alc <- weekly_crime %>% 
  ...
  feols(alcohol_offense_per25 ~ beta_lead_binned + beta_minus_7 + beta_minus_6 + beta_minus_5 + beta_minus_4 + beta_minus_3 + beta_minus_2 + beta_0 +
          ...
          beta_plus_6 + beta_plus_7 + beta_lag_binned| university + semester_number , cluster = ~university, data = .) 
es_sex <- weekly_crime %>% 
  group_by(university, semester_number) %>% 
  mutate(university_by_semester_number = cur_group_id()) %>% 
  ungroup() %>% 
