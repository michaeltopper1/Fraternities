...
sex_p <- daily_crime %>% 
  fepois(sexual_assault_per25 ~ treatment |
          uni_semester + weekday, cluster = ~university, data = .)

alc_p <- daily_crime %>% 
  fepois(alcohol_offense_per25 ~  treatment  |
          uni_semester + weekday, cluster = ~university, data = .)

sex_weekdays_p <- daily_crime_weekdays %>% 
  fepois(sexual_assault_per25 ~ treatment |
          uni_semester + weekday, cluster = ~university, data = .)

alc_weekdays_p <- daily_crime_weekdays %>% 
  fepois(alcohol_offense_per25 ~  treatment  |
          uni_semester + weekday, cluster = ~university, data = .)

sex_weekends_p <- daily_crime_weekends %>% 
  fepois(sexual_assault_per25 ~ treatment |
          uni_semester + weekday, cluster = ~university, data = .)

alc_weekends_p <- daily_crime_weekends %>% 
  fepois(alcohol_offense_per25 ~  treatment  |
          uni_semester + weekday, cluster = ~university, data = .)
