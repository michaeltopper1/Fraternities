...
Assume the researcher observes $Y_{0,t}=(1-w_{0,t})y_{0,t}(0) + w_{0,t}y_{0,t}(1)$. This means the researcher observes an untreated Great Britain before 2016 and a treated Great Britain 2016 onward. The researcher also observes $Y_{j,t}=y_{j,t}(0)$ for all $j=1,...,J$ and $t=1,...T$, implying the control units are never treated.

The goal is to determine the treatment effect of the policy change. The treatment effect for the treated observation at period *t* is defined as $\alpha_{0,t}=y_{0,t}(1)-y_{0,t}(0)$. This would be comparing the GDP of Great Britain where Brexit passed to the GDP of Great Britain where Brexit did not pass in period t. The researchers observe $y_{0,t}(1)$ when $t \ge T_0$. Thus, $y_{0,t}(0)$ must be estimated when $t \ge T_0$.
...
\begin{align}
Y_{0,t}&= \sum_{j=1}^{J+1} \beta_{j,t}Y_{j,t}+\epsilon_t & \epsilon_t|\sigma^2 \sim N(0, \sigma^2) &\\
\beta_{j,t}&=\beta_{j,t-1}+\eta_{j,t} & \eta_{j,t} \sim N(0,\theta_j) &\ \ \ \  \forall j\\
\beta_{j,0}&\sim N(\beta_j,\theta_j P_{jj}) & &\ \ \ \ \forall j
\end{align}

where $Y_{J+1,t}=1$ for all t (intercept). In the state space literature, equation (1) is known as the *observation equation* and equation (2) is referred to as the *state equations*. Setting $\beta_{j,t}=\beta_j$ and allowing for a local linear time trend will yield the original estimator in @brodersen_inferring_2015.
...
### Sample $\beta$ and $\sqrt{\theat}$

Block draw $\beta$ and $\sqrt{\theta}$ from the normal conditional posterior:

\begin{align}
 \mathcal{N}_{J+1}  \left((\tilde{Y}^T\tilde{Y} + \sigma^2 V^{-1})^{-1}\tilde{Y^T}{Y_0}, \sigma^2 (\tilde{Y}^T\tilde{Y} + \sigma^2 V^{-1})^{-1}  \right)
 \end{align}
...
\begin{align}
V_{\beta}= diag[\lambda^2 \tau_{1}^2,\lambda^2\tau_{2}^2,...,\lambda^2\tau_{J+1}^2, \kappa^2\xi_1^2,\kappa^2\xi_2^2,\dots,\kappa^2\xi_{J+1}^2]
\end{align}
...
\begin{align}
 \mathcal{N}_{J+1}\left((\bar{Y^T}\bar{Y} + \frac{\sigma^2}{\kappa^2} V_{\sqrt{\theta}}^{-1})^{-1}\bar{Y^T}\bar{Y_0}, \sigma^2 (\bar{Y^T}\bar{Y} + \frac{\sigma^2}{\kappa^2} V_{\sqrt{\theta}}^{-1})^{-1}  \right)
 \end{align}
...
\tilde{\beta}_{1,1}Y_{1,1} & \tilde{\beta}_{2,1}Y_{2,1} & \dots & \tilde{\beta}_{J+1,1}Y_{J+1,1}\\
\vdots & \vdots & \vdots & \vdots \\
\tilde{\beta}_{1,T_0-1}Y_{1,T_0-1} & \tilde{\beta}_{2,T_0-1}Y_{2,T_0-1} & \dots & \tilde{\beta}_{J+1,T_0-1}Y_{J+1,T_0-1}
\end{array}\right)
\end{align}

\begin{align}
V_{\sqrt{\theta}}=diag[\xi_{1}^2,\xi_{2}^2,...,\xi_{J+1}^2]
\end{align}

\begin{align}
\bar{Y_0}=\left(\begin{array}{c} 
Y_{0,1}-\sum_{j=1}^{J+1}{\beta}_{j,1}Y_{j,1} \\
Y_{0,2}-\sum_{j=1}^{J+1}{\beta}_{j,2}Y_{j,2} \\
\vdots \\
Y_{0,T_0-1}-\sum_{j=1}^{J+1}{\beta}_{j,T_0-1}Y_{j,T_0-1} 
\end{array}\right)
\end{align}
...
$$
\begin{aligned}
f(Y | \beta, \sqrt{\theta},& \sigma^2)\pi(\sigma^2) \pi(\lambda^2)\pi(\kappa^2) \prod_{j=1}^{J+1} \pi(\beta_j | \tau_j^2) \pi(\tau_j^2) \pi( \sqrt{\theta_j} | \xi_j^2) \pi(\xi_j^2)  =\\
...
$$exp\left\{\frac{-1}{2\sigma^2}\left(Y-X\beta - (X*\tilde{\beta})\sqrt{\theta}\right)^T \left(Y-X\beta - (X*\tilde{\beta})\sqrt{\theta}\right) \right\} \prod_{j=1}^{J+1} exp\left\{ \frac{-1}{(2 \tau_j^2\lambda^2)} \beta_j^2 \right\}$$

Combining exponents yields:

$$exp\left\{\frac{-1}{2\sigma^2}\left(Y-X\beta - (X*\tilde{\beta})\sqrt{\theta}\right)^T \left(Y-X\beta - (X*\tilde{\beta})\sqrt{\theta}\right) + \sum_{j=1}^{J+1}\frac{-\sigma^2}{(2\tau_j^2 \lambda^2)}\beta_j^2 \right\}$$
...
where $D=diag(\tau_1^2,...\tau_{J+1}^2)$. Multiplying out and rearranging yields:
...
with $B=(x*\tilde{\beta})^T(x*\tilde{\beta}) +\frac{\sigma^2}{\kappa^2}diag[\xi_1^2,...\xi_{J+1}^2]^{-1}$.
