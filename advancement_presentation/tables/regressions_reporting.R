...
reporting_table_alc <- ifc::main_table(last_panel = lag_alc) %>% 
  ...
  slice(1:4) %>% 
  gt() %>% 
  cols_label(term = " ", `Model 1` = "More than 1-Day Lag", `Model 2` = "More than 3-Day Lag", `Model 3` = "More than 7-Day Lag",
             `Model 4` = "More than 14-Day Lag") %>% 
  tab_header(title = "Effect of Moratoriums on Changes in Reporting: Alcohol Offenses") %>% 
  tab_spanner(label = "Reporting Lag", columns = c(2:5)) %>% 
  tab_source_note("Standard errors clustered by university. A reporting lag is defined as an offense that was reported more than 1 (Column 1), 3 (Column 2), 7 (Column 3), or 14 (Column 4) days after it occurred. 33 of the 38 universities have information on date occurred.")
  
reporting_table_sex <- ifc::main_table(last_panel = lag_sex) %>% 
  add_row(term = "Mean of Dependent Variable", 
  ...
  cols_label(term = " ", `Model 1` = "More than 1-Day Lag", `Model 2` = "More than 3-Day Lag", `Model 3` = "More than 7-Day Lag",
             `Model 4` = "More than 14-Day Lag") %>% 
  tab_header(title = "Effect of Moratoriums on Changes in Reporting: Sexual Assaults") %>% 
  tab_spanner(label = "Reporting Lag", columns = c(2:5)) %>% 
  tab_source_note("Standard errors clustered by university. A reporting lag is defined as an offense that was reported more than 1 (Column 1), 3 (Column 2), 7 (Column 3), or 14 (Column 4) days after it occurred. 33 of the 38 universities have information on date occurred.")
