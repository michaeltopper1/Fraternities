...
week_before_after_graph_alc <- weeksplit_controls %>%
  ...
  labs(x = "Coefficient Estimate and 95% Confidence Interval", y = "Time Before/After/In Moratorium") +
  coord_flip()

week_before_after_graph_sex <- weeksplit_controls %>%
  mutate(time_grid = case_when(
    time == "Week Before" ~-1,
    time == "In\nMoratorium" ~0,
    time == "Week After" ~ 1
