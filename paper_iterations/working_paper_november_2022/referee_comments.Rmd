...
In addition to this change and other suggestions from the referees, I have updated the nomenclature of the original model in Equation \ref{main_model} shown on page BLANK. The treatment variable now reads $InMoratorium_{u,t}$ rather than $Moratorium_{u,t}$ to more consistently match with the tables and figures. The equation now reads:

\begin{equation}\label{main_model}
Y_{u,t} = \beta InMoratorium_{u,t}  + \gamma_{u} + \lambda \mathbb{X}_{t} +  \epsilon_{u,t}
\end{equation}

Finally, below is a changelog of the major refinements from the first draft to this current draft. While all of these changes are discussed more in the responses to the referees, they are also here, in order of occurrence, for your convenience:

\noindent **Changelog:**

* Introduction
  + Updated the Introduction with slight broadening of literature to capture a wider audience (see Section \ref{section:editor}).
* Background
  + No updates.
* Data
 + Significantly condensed two subsections by over 500 words: Section 3.1 (Sample Construction) and 3.2 (Matching and Harmonization). Extraneous details and footnotes have now been removed and placed into a new appendix on data cleaning (Appendix \ref{asection:data}).
 + A sentence regarding the representativeness of the sample in regards to fraternity life is added to Section 3.3. 
* Estimation
  + The assumptions now include an important discussion that the moratorium itself, rather than the triggering event (such as a death), is the driving contributor of the effects. Alludes to the new analysis later on in the paper.
  + Equation 1 now reads $InMoratorium_{u,t}$ rather than $Moratorium_{u,t}$ to better align with the tables and figures. The estimation methods have not changed, only the nomenclature.
* Main Results
  + Discusses a weighted regression as a robustness check and refers reader to the weighted regression table in the appendix.
  + Clarification on the Campus Safety and Security Data has been added in regards to arrests/disciplinary actions in Section 5.2. A small discussion of the results has also been added. (Referee #1)
  + A new subsection (5.4: Is a Moratorium Effective Across its Entire Implementation?) has been added. Discusses the dynamic effects within a moratorium.
* Heterogeneity
  + An additional couple sentences have been added in Section 6.1 to demonstrate that party schools have higher alcohol prevalence outside of a moratorium. (Referee #3)
  + In Section 6.2, a new test to show that the moratorium, rather than the triggering event, is causing the estimated effects has been added while the previous weaker test has been removed. (Referees #3, #4)
* Discussion
  + No major changes.
* Conclusion
  + Removed any language referring to the results as large. 
* Figures
  + All figures have been edited to make the text slightly larger and more readable.
  + Figure \ref{} added for dynamic effects within a moratorium.
* Tables
  + Table \ref{} now includes the pre-treatment mean.
* Appendices
  + New Appendix \ref{}. 
  + Appendix \ref{} edited to include further analysis of Campus Safety and Security Data arrests. Corresponding table also updated.
  + All figures have been edited to make the text slightly larger and more readable.
  + Figure \ref{} added for analysis of death-but-no-moratorium schools.
  + Figure \ref{} added for representativeness of fraternity life at each school.
  + Table \ref{} added for WLS regressions.
...
> However, there is little evidence of an effect on liquor law arrests as shown in Columns 4 and 5—consistent with the literature that campus police do not typically arrest students for alcohol violations [@bernat_college_2014]. As discussed in Section \ref{section:spillover}, this supports the notion that if moratoriums displace alcohol-fueled behavior, they displace it to *less* risky areas whereby behavior can more easily be intervened before it becomes dangerous.
...
Could add to 6.1 party schools:


Table 7 shows that party schools exhibit larger decreases in alcohol offenses than non- party schools. The point estimates in Panel A indicate that moratoriums decrease alcohol offenses on academic-calendar days by approximately 33% from the mean for party schools and 16% for non-party schools. Importantly, only the point estimates for party schools are statistically significant, thereby suggesting that the effects of the moratorium are driven by schools that have a stronger party culture.
...
Thank you for this thoughtful suggestion. I discuss this analysis in the response to Referee #2, as they had a similar recommendation.
