...
party_school_table <- ifc::main_table(alc_party, last_panel = sex_party) %>% 
  ...
  footnote(list("Standard errors are clustered by university and each offense is defined as per-25000 enrolled students. The column `All Schools' represents specification (2) from the main results table. Party schools were determined from niche.com's list of top partying schools. A university in the top 50 is considered a party school which amounts to 16/38 universities.  Holiday controls include controls for Veterans Day, Thanksgiving, Labor Day, Halloween, and MLK Day. Christmas/New Years/July 4th are not included since no university's academic calendar contains them. A moratorium is a temporary halt on fraternity-related activities with alcohol.",
