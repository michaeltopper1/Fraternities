...
data_used <- tibble(attributes = c("Source of Data:","Reporting Mandate:", "Level of Aggregation:", "Fraction Reporting Consistently:", "Alcohol Violations:", 
                    ...
                    nibrs = c("FBI", "Voluntary", "Incident-level", as.character(round(9/37,2)), "Arrests only", "All incidences reported",
                               "No", "Spillovers of partying"),
                    ucr = c("FBI", "Voluntary", "Monthly", as.character(round(29/37, 2)), "None","Hierarchy rule",
                             "No", "Not used"))
data_used <-  kbl(data_used, booktabs = T, digits = 3,
  ...
  pack_rows("Source and Requirement:", 1, 2, bold = F, italic = T) %>% 
  pack_rows("Aggregation and Consistency:", 3, 4, bold = F, italic = T, latex_gap_space = "0.5cm") %>% 
  pack_rows("Offenses Reported and Location:", 5, 8,  bold = F, italic = T, latex_gap_space = "0.5cm") %>% 
