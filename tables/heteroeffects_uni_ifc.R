...
ifc_uni_table <- ifc::main_table(uni_regs_alc, uni_regs_sex,
  ...
  kbl(booktabs = T, col.names = c(" ", "(1)", "(2)", "(3)"), align = "lccc",
      caption = "\\label{ifc_uni_table}Effect of Moratoriums Imposed by the University vs. the IFC (OLS)") %>% 
kable_styling(font_size = 11) %>% 
  ...
  row_spec(12, hline_after = F) %>% 
  footnote(list("Standard errors clustered by university. In Panel A, the In Moratorium is interacted with an indicator variable equal to one if the moratorium was enacted by a university. In Panel B, In Moratorium is interacted with an indicator variable equal to one if the moratorium was enacted by the IFC. Controls follow the preferred specification from Column 2 in the main results table with day of week, holiday, semester, football game-day, and university by academic year fixed effects. Panel A shows the effects of a moratorium when a moratorium is imposed by the university. University-imposed moratoriums represent 27/44 (61%) of the moratoriums. Panel B shows the effects of a moratorium when the IFC council imposes the moratorium. This is a student-lead initiative. IFC-imposed moratoriums represent 17/44 (39%) of the moratoriums in the sample. Weekends represent Fridays through Sundays while Weekdays represent Mondays through Thursdays.",
                "* p < 0.1, ** p < 0.05, *** p < 0.01"),
           threeparttable = T)
