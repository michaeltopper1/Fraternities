...
for (month in months){
  print(month)
  file_names <- dir(paste(directory,month, "-2019/",sep = ''), pattern = "9.rtf")
  for (i in seq_along(file_names)) { ### for every single file in here
    print(file_names[i])
    indiana <- read_rtf(paste(directory,month,"-2019/", file_names[i], sep = ""))
...
crime_total_2019 <- crime_total %>%
  select(-value) %>%
  mutate(university = "Indiana University-Bloomington")

save(crime_total_2019, file = "/Users/michaeltopper/Desktop/Fraternities and Sexual Assault/Data/campus_daily_crime_log/Indiana University/crime_2019.rda")

write_lines(problem_files, file = paste(directory, "problem_files_2019.txt"), sep = "\n")
