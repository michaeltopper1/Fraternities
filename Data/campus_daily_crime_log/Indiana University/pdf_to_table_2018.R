...
for (month in months){
  print(month)
  file_names <- dir(paste(directory,month, "-2018/",sep = ''), pattern = "8.rtf")
  for (i in seq_along(file_names)) { ### for every single file in here
    print(file_names[i])
    indiana <- read_rtf(paste(directory,month,"-2018/", file_names[i], sep = ""))
...
crime_total_2018 <- crime_total %>%
  select(-value) %>%
  mutate(university = "Indiana University-Bloomington")

save(crime_total_2018, file = "/Users/michaeltopper/Desktop/Fraternities and Sexual Assault/Data/campus_daily_crime_log/Indiana University/crime_2018.rda")

write_lines(problem_files, file = paste(directory, "problem_files_2018.txt"), sep = "\n")
