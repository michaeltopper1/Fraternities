...
for (month in months){
  print(month)
  file_names <- dir(paste(directory,month, "-2015/",sep = ''), pattern = "5.pdf")
  for (i in seq_along(file_names)) { ### for every single file in here
    print(file_names[i])
    indiana <- pdf_text(paste(directory,month,"-2015/", file_names[i], sep = ""))
...
crime_total_2015 <- bind_rows(crime_total, crime_total_r2k) %>% 
  select(-value) %>% 
  mutate(university = "Indiana University-Bloomington")

save(crime_total_2015, file = "/Users/michaeltopper/Desktop/Fraternities and Sexual Assault/Data/campus_daily_crime_log/Indiana University/crime_2015.rda")

write_lines(problem_files, file = paste(directory, "problem_files_2015.txt"), sep = "\n")
