...
for (month in months){
  print(month)
  file_names <- dir(paste(directory,month, "-2014/",sep = ''), pattern = "4.pdf")
  for (i in seq_along(file_names)) { ### for every single file in here
    print(file_names[i])
    indiana <- pdf_text(paste(directory,month,"-2014/", file_names[i], sep = ""))
...
crime_total_2014 <- bind_rows(crime_total_r2k, crime_total)
crime_total_2014 <- crime_total_2014 %>% 
  select(-value) %>% 
  mutate(university = "Indiana University-Bloomington")

save(crime_total_2014, file = "/Users/michaeltopper/Desktop/Fraternities and Sexual Assault/Data/campus_daily_crime_log/Indiana University/crime_2014.rda")

write_lines(problem_files, file = paste(directory, "problem_files_2014.txt"), sep = "\n")
