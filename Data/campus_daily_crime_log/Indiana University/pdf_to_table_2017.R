...
for (month in months){
  print(month)
  file_names <- dir(paste(directory,month, "-2017/",sep = ''), pattern = "7.pdf")
  for (i in seq_along(file_names)) { ### for every single file in here
    print(file_names[i])
    indiana <- pdf_text(paste(directory,month,"-2017/", file_names[i], sep = ""))
...
crime_total_2017 <- crime_total %>%
  select(-value) %>%
  mutate(university = "Indiana University-Bloomington")

save(crime_total_2017, file = "/Users/michaeltopper/Desktop/Fraternities and Sexual Assault/Data/campus_daily_crime_log/Indiana University/crime_2017.rda")

write_lines(problem_files, file = paste(directory, "problem_files_2017.txt"), sep = "\n")
